...
class EmailOutreach(OutreachNode):
    ...
    NODE_NAME = 'Email Outreach'

    def __init__(self):
        super(EmailOutreach, self).__init__(has_output=True)

        self.template_data = {}
        self.set_property('color', (254, 216, 177))
    ...
    def set_template_id_from_name(self):
        ...
        template_name = self.get_property('email_templates')
        ...
        if template_id:
            for template_name, template_details in self.template_data.items():
                if template_details['id'] == template_id:
                    self.set_property('email_templates', template_name)
                    return None
    ...
    def get_display_info(self, node_templates_root='./data/templates'):
        ...
        selected_template = self.get_property('email_templates')
        ...
        display_text = f"""
            <h3>Email Template: {selected_template}</h3>
    ...
    def validate_has_template_selected(self):
        ...
        if self.template_data == {}:
            raise ValueError(
                'Template data is not set. Ensure that at least one email template exists for this workflow category.'
            )
